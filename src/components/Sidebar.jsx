    ...
    <>
      ...
      >
        ...
        <div className="px-3 pb-2">
          <button
            onClick={onNewChat}
            className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-accent text-sm font-medium cursor-pointer transition-all duration-200"
            style={{
            ...
            return (
              <SpotlightCard key={chat.id} className="rounded-xl" spotlightColor="rgba(124,92,252,0.12)">
                <div
                  ...
                  className={`group flex items-center gap-2 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 text-[13px] ${
                    isActive ? 'text-zinc-100' : 'text-zinc-500 hover:text-zinc-300'
                  }`}
                  style={{
                    background: isActive ? 'rgba(124,92,252,0.1)' : 'transparent',
                    borderLeft: isActive ? '2px solid rgba(124,92,252,0.6)' : '2px solid transparent',
                  }}
                >
                  <svg className={`w-4 h-4 shrink-0 transition-colors ${isActive ? 'text-accent' : 'text-zinc-600'}`} fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
                  </svg>
                  <span className="flex-1 truncate">{chat.title}</span>
                  <button
                    onClick={(e) => { e.stopPropagation(); onDeleteChat(chat.id); }}
                    className="opacity-0 group-hover:opacity-100 touch-show w-6 h-6 flex items-center justify-center rounded-md text-zinc-600 hover:text-red-400 transition shrink-0 cursor-pointer"
        ...
        <div className="p-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm text-zinc-500 hover:text-zinc-300 transition-all duration-200 cursor-pointer"
            style={{ background: 'transparent' }}
